...
export default function PlacementsPage() {
  ...
  const { data, isLoading } = usePlacements({ page, limit: 10 });
  ...
  if (!data?.data.length) return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground">Your placement records</p>
      </div>
      <EmptyState icon={CheckSquare} title="No placements yet" description="Placements will appear here once you've been placed." />
    </div>
  );
              ...
              <div className="flex items-start justify-between gap-3">
                <div>
                  ...
                  <div className="mt-1 flex items-center gap-2 text-sm text-muted-foreground">
                    ...
                    <span>{p.application?.job?.employer?.companyName}</span>
                  </div>
